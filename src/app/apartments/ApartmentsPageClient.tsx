...
export default function ApartmentsPageClient({ initialProperties }: { initialProperties: any }) {
    ...
    useEffect(() => {
        if (initialProperties && initialProperties[language]) {
            setAllApartments(initialProperties[language])
            setFilteredApartments(initialProperties[language])
                                ...
                                {totalPages > 1 && (
                                    <div className="mt-8 flex flex-col sm:flex-row items-center justify-between gap-4">
                                        ...
                                        <div className="text-sm text-gray-600">
                                            {language === "en" ? `Showing ${startIndex + 1}-${Math.min(endIndex, filteredApartments.length)} of ${filteredApartments.length} properties` :
                                                language === "ru" ? `Показано ${startIndex + 1}-${Math.min(endIndex, filteredApartments.length)} из ${filteredApartments.length} объектов` :
                                                    language === "pl" ? `Pokazano ${startIndex + 1}-${Math.min(endIndex, filteredApartments.length)} z ${filteredApartments.length} nieruchomości` :
                                                        language === "fr" ? `Affichage de ${startIndex + 1}-${Math.min(endIndex, filteredApartments.length)} sur ${filteredApartments.length} propriétés` :
