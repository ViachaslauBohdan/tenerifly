...
export default function ToursPageClient({ initialTours }: { initialTours: any }) {
    ...
    useEffect(() => {
        if (initialTours && initialTours[language]) {
            setTours(initialTours[language])
            setInitialLoadComplete(true)
        }
    }, [initialTours, language])
                                ...
                                {totalPages > 1 && (
                                    <div className="mt-8 flex flex-col sm:flex-row items-center justify-between gap-4">
                                        ...
                                        <div className="text-sm text-gray-600">
                                            {language === "en" ? `Showing ${startIndex + 1}-${Math.min(endIndex, tours.length)} of ${tours.length} tours` :
                                                language === "ru" ? `Показано ${startIndex + 1}-${Math.min(endIndex, tours.length)} из ${tours.length} экскурсий` :
                                                    language === "pl" ? `Pokazano ${startIndex + 1}-${Math.min(endIndex, tours.length)} z ${tours.length} wycieczek` :
                                                        language === "fr" ? `Affichage de ${startIndex + 1}-${Math.min(endIndex, tours.length)} sur ${tours.length} excursions` :
