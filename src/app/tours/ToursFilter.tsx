        ...
        const loadFilterOptions = async () => {
            try {
                ...
                if (data.data && Array.isArray(data.data)) {
                    ...
                    const cities = [...new Set(tours
                        .map((tour: TourData) => tour.location?.city)
                        .filter((city): city is string => Boolean(city) && typeof city === 'string')
                    )].sort()

                    const regions = [...new Set(tours
                        .map((tour: TourData) => tour.location?.region)
                        .filter((region): region is string => Boolean(region) && typeof region === 'string')
                    )].sort()

                    const languages = [...new Set(tours
                        .map((tour: TourData) => tour.language)
                        .filter((language): language is string => Boolean(language) && typeof language === 'string')
                    )].sort()
    ...
    return (
        <div className="lg:w-80">
            <div className="bg-white rounded-lg shadow-sm border p-6 max-h-screen overflow-y-auto">
                ...
                {/* Тип развлечения */}
                <div className="mb-4">
                    <label className="block text-sm font-medium text-gray-700 mb-2">{t.tourType}</label>
                    <select
                ...
                {/* Доступные даты */}
                <div className="mb-4">
                    <label className="block text-sm font-medium text-gray-700 mb-2">{t.availableDates}</label>
                    <input
                        type="date"
                ...
                {/* Язык гида */}
                <div className="mb-4">
                    <label className="block text-sm font-medium text-gray-700 mb-2">{t.guideLanguage}</label>
                    <select
                ...
                {/* Категория экскурсии */}
                <div className="mb-4">
                    <label className="block text-sm font-medium text-gray-700 mb-2">{t.tourCategory}</label>
                    <select
                ...
                {/* Размер группы */}
                <div className="mb-4">
                    <label className="block text-sm font-medium text-gray-700 mb-2">{t.groupSize}</label>
                    <select
                ...
                {/* Включено */}
                <div className="mb-6">
                    <label className="block text-sm font-medium text-gray-700 mb-3">{t.included}</label>
